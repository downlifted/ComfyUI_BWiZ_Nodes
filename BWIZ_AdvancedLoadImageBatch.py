...
class BWIZ_AdvancedLoadImageBatch:
    ...
    def load_batch_images(self, seed, max_value, path, natural_sort='true', path_type='directory', sort_order='numerical', filename_text_extension='true'):
        ...
        if sort_order == 'random':
            BWIZ_AdvancedLoadImageBatch.current_index = random.randint(0, len(fl.image_paths) - 1)
        else:
            BWIZ_AdvancedLoadImageBatch.current_index = seed % len(fl.image_paths)  # Ensure index wraps around

        image, filename = fl.get_image_by_id(BWIZ_AdvancedLoadImageBatch.current_index)

        # Increment the index for the next run
        BWIZ_AdvancedLoadImageBatch.current_index = (BWIZ_AdvancedLoadImageBatch.current_index + 1) % max_value
        ...
        return (image_tensor, filename, BWIZ_AdvancedLoadImageBatch.current_index, width, height)
    ...
    def IS_CHANGED(cls, **kwargs):
        fl = BWIZ_AdvancedLoadImageBatch.NaturalSortLoader(kwargs['path'], kwargs['sort_order'], kwargs['path_type'] == 'url_directory')
